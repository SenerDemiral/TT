   ...
   partial class TrnMsb : Json
	{
		...
		public void RefreshTurnuvaMusabaka()
		{
			...
			var TM = Db.SQL<TTDB.Musabaka>("SELECT tt FROM Musabaka tt WHERE tt.Turnuva = ? ORDER BY Trh", trnObj);
		...
		void Handle(Input.GetMaclar action)
		{
			...
			maclar.Heading = $"{musabaka.HomeTakim.Ad} - {musabaka.GuestTakim.Ad} Maçları";
			maclar.Data = null;
			RecentMaclar = maclar;
		}
